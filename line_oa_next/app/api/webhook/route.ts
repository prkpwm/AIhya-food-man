...
function liff(page: string): string {
  return `${env.liffUrl}?page=${page}`;
}

function buildMenuFlex(menu: Menu): LineMessage {
  return { type: 'flex', altText: menu.name, contents: { type: 'bubble', ...(menu.imageUrl ? { hero: { type: 'image', url: menu.imageUrl, size: 'full', aspectRatio: '20:13', aspectMode: 'cover' } } : {}), body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: menu.name, weight: 'bold', size: 'xl' }, { type: 'text', text: `฿${menu.price.toFixed(0)}`, color: '#FF6B00', size: 'lg', weight: 'bold' }, { type: 'text', text: menu.description, size: 'sm', color: '#999999', wrap: true }] }, footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [{ type: 'button', style: 'primary', color: '#FF6B00', action: { type: 'uri', label: '🛒 สั่งเลย', uri: liff('order') } }, { type: 'button', style: 'secondary', action: { type: 'uri', label: '📋 ดูเมนูทั้งหมด', uri: liff('order') } }] } } } as unknown as LineMessage;
}
...
function buildOrderStatusFlex(order: Order): LineMessage {
  ...
  return { type: 'flex', altText: `สถานะ #${order.id.slice(-6)}`, contents: { type: 'bubble', body: { type: 'box', layout: 'vertical', spacing: 'md', contents: [{ type: 'text', text: 'สถานะออเดอร์', weight: 'bold', size: 'lg' }, { type: 'text', text: `#${order.id.slice(-6)}`, size: 'sm', color: '#999999' }, { type: 'separator' }, { type: 'text', text: statusLabel[order.status] ?? order.status, size: 'xl', weight: 'bold' }, ...(order.estimatedWaitMinutes > 0 ? [{ type: 'text' as const, text: `⏱ รออีก ~${order.estimatedWaitMinutes} นาที`, size: 'sm' as const, color: '#999999' }] : []), { type: 'text', text: `ยอดรวม ฿${order.totalPrice.toFixed(0)}`, size: 'sm', color: '#555555' }] }, footer: { type: 'box', layout: 'vertical', spacing: 'sm', contents: [{ type: 'button', style: 'primary', color: '#FF6B00', action: { type: 'uri', label: '📦 ดูสถานะ', uri: liff('status') } }, { type: 'button', style: 'secondary', action: { type: 'uri', label: '🍽️ สั่งอาหาร', uri: liff('order') } }] } } } as unknown as LineMessage;
}
