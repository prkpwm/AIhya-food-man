...
// ─── GET /order-web/app.js — client-side script ───────────────────────────────

router.get('/app.js', (_req: Request, res: Response): void => {
  res.setHeader('Content-Type', 'application/javascript');
  res.send(orderWebScript());
});
...
router.get('/', (_req: Request, res: Response): void => {
  ...
  const menuCards = menus.map((m) => {
    ...
    return '<div class="menu-card" onclick="openDetail(\'' + m.id + '\')">'
      + img
      + '<div class="menu-info">'
  ...
  res.send(orderWebHtml(menuCards, menusJson));
});
...
function orderWebHtml(menuCards: string, menusJson: string): string {
  return '<!DOCTYPE html>'
    + '<html lang="th"><head>'
    + '<meta charset="UTF-8"/>'
    ...
    + '<style>' + orderWebCss() + '</style>'
    + '</head><body>'
    + '<div class="header"><h1>🍽️ สั่งอาหาร</h1><p>แตะเมนูเพื่อเลือกและปรับแต่ง</p></div>'
...
// ─── JS served separately to avoid template literal conflicts ─────────────────

function orderWebScript(): string {
  return `
...
function renderSheet() {
  ...
  if (m.addons && m.addons.length > 0) {
    ...
    m.addons.forEach(function(a) {
      ...
      arows += '<div class="option-row" onclick="toggleAddon(' + JSON.stringify(a.id) + ')">'
  ...
  if (m.portionOptions && m.portionOptions.length > 0) {
    ...
    m.portionOptions.forEach(function(p) {
      ...
      prows += '<div class="option-row" onclick="selectPortion(' + JSON.stringify(p.id) + ')">'
...
function calcTotal() {
  ...
  if (m.addons) {
    m.addons.forEach(function(a) { if (sheetAddons[a.id]) price += a.price; });
  }
...
async function checkout() {
  ...
  try {
    var res = await fetch('/order-web/confirm', {
      method: 'POST',
    ...
    var flex = {
      type: 'flex',
      altText: 'ยืนยันออเดอร์ #' + d.shortId + ' — ฿' + d.totalPrice.toFixed(0),
      contents: {
        type: 'bubble',
